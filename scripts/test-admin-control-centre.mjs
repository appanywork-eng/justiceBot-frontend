...
assert.match(app, /\(!adminActive \|\| adminDraftOpen\) && \(\s*<section id="draft-petition">/);
assert.match(app, /!adminActive && \(\s*<FreeAccessPanel/);
...
assert.match(controlCentre, /onDraftPetition/);
assert.match(controlCentre, /Draft Petition/);
assert.match(app, /onDraftPetition=\{openAdminPetitionStudio\}/);
assert.match(app, /"x-admin-token":\s*adminToken/);
assert.match(app, /adminActive && data\.admin === true && data\.tx_ref/);
assert.match(app, /await unlockByTxRef\(data\.tx_ref\)/);
assert.match(app, /Generate Full Petition — Unlimited Admin Access/);
assert.match(app, /Draft Another Petition/);
assert.match(app, /!adminActive &&\s*accessStatus\.enabled &&\s*!verifiedUser/);
...
console.log("✅ ADMIN MODE EXCLUDES PUBLIC VERIFIED-ACCESS, PAYMENT AND MARKETING UI");
console.log("✅ ADMIN NAVIGATION AND MOBILE ACTIONS ARE ADMIN-ONLY AND RESPONSIVE");
console.log("✅ ADMINISTRATORS CAN GENERATE AND UNLOCK UNLIMITED FULL PETITIONS");
console.log("✅ ADMIN PETITION STUDIO BYPASSES PUBLIC EMAIL VERIFICATION AND PAYMENT");
console.log("✅ ADMINISTRATORS CAN DRAFT ANOTHER PETITION WITHOUT LEAVING ADMIN MODE");
